...
export interface EnemyTooltipProps {
  enemy: Enemy;
  vx: number;
  vy: number;
  viewWidth: number;
...
export function EnemyTooltip({ enemy, vx, vy, viewWidth, viewHeight }: EnemyTooltipProps) {
  const TILE_PX = 32;
  const POPUP_W = 160;
          ...
          <div className="min-w-0">
            ...
            {enemy.isEcho && (
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <span className="text-muted-foreground" style={{ fontSize: 8 }}>faded boss — diminished but deadly</span>
              </div>
            )}
          </div>
        </div>
